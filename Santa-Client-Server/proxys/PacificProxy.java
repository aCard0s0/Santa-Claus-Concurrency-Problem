 ...
 */
public class PacificProxy extends Thread{

    private static int nProxy = 0;           // Contador de threads lançados
    private final ServerCom sconi;                // Canal de comunicação
    private final PacificInterface southInter;    // Interface ao Stable
    private ReindeerStates state;
    
    public PacificProxy(ServerCom sconi, PacificInterface southInter){
        //super("ProxyStable_"+ SouthPacificProxy.getProxyId());
        this.sconi = sconi;
        this.southInter = southInter;
    }
    ...
    public void run(){
        ...
        try { 
            outMsg = southInter.processAndReply(inMsg);         // processá-lo
    ...
    private static int getProxyId () {
        ...
        try {
            cl = Class.forName ("servers.StableProxy");
